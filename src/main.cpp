...
int main() {
    ...
    std::cout << "Welcome to the our Premier League Match Fixture Generator!\n";
    std::cout << "--------------------------------------------------------\n\n";//here we are creating a more user-friendly information

    // Step 1: Here we now Load our teams from the CSV file
    ...
    // Step 3: Convert matches to string for easier display and saving
    ...
    // Step 4: Display fixtures
    std::cout << "\nFixtures successfully generated! Displaying fixtures:\n";
    Utils::printFixtures(fixtureStrings);

    // Ask the user if they want to save the fixtures to a file
    char saveOption;
    std::cout << "\nWould you like to save the fixtures to a file? (y/n): ";
    std::cin >> saveOption;

    if (saveOption == 'y' || saveOption == 'Y') {
        std::string outputFilename = "../build/generated_fixtures.csv"; // Save in the build folder
    ...
    std::cout << "Thank you for using the ABC Premier League Match Fixture Generator!\n";
    return 0;
}
